    ...
    func getDataSource(_ fetchRequest: NSFetchRequest<NSFetchRequestResult>?) -> DATASource? {
        ...
        let dataSource = DATASource(tableView: tableView, cellIdentifier: "Cell", fetchRequest: request!, mainContext: CardMagus.sharedInstance.dataStack!.mainContext, configuration: { cell, item, indexPath in
            if let set = item as? CMSet {
                ...
                if let image = CardMagus.sharedInstance.imageFromBundle("/images/set/\(set.code!)/C/32.png") {
                    cell.imageView?.image = image
                } else if let image = CardMagus.sharedInstance.imageFromBundle("/images/set/\(set.code!)/R/32.png") {
                    cell.imageView?.image = image
                } else if let image = CardMagus.sharedInstance.imageFromBundle("/images/other/blank/32.png") {
                    cell.imageView?.image = image
                }
